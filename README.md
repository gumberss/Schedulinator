...
- [ ] If the timeout exceeds,
  - [ ] Calculate the next execution time based on backoff + jitter
  - [ ] Execute the task again
  - [ ] If the retries limit was reached
    - [ ] Change the next execution to the execution time plus the worst case of the retry policy
    - [ ] Unlock the task

### Unlock tasks
- [ ] Every X time lock a key on Redis
- [ ] Get the top 10 tasks with negative values that are stuck for more than 30 seconds
- [ ] Set the next execution time to now
- [ ] Unlock them on Redis with the new execution time
